...
{
    /* 创建一个用于保存接收数据包（ReceiveDataPacket_t）的队列，最多可容纳 15 个元素�?*/
...
/**
 * @brief 计算 CRC-8 校验值（多项�?0x07，初始�?0x00）的封装函数
 */
static uint8_t CalculateCRC8(const uint8_t *data, uint16_t length) { /* 封装�?CRC8 计算函数定义 */ // 每行注释
    uint8_t crc = 0x00;                                           /* 初始�?CRC 值为 0x00 */ // 每行注释
    for (uint16_t i = 0; i < length; i++) {                       /* 遍历所有输入字�?*/ // 每行注释
        crc ^= data[i];                                           /* 与当前字节异�?*/ // 每行注释
        for (uint8_t bit = 0; bit < 8; bit++) {                   /* 对每个位进行处理 */ // 每行注释
            if (crc & 0x80)                                       /* 如果最高位�?1 */ // 每行注释
            {
                crc = (uint8_t)((crc << 1) ^ 0x07);              /* 左移并按多项式异�?*/ // 每行注释
            }
            else
            {
                crc <<= 1;                                       /* 仅左移一�?*/ // 每行注释
...
{
    uint8_t computedCRC = 0;                                        /* 计算得到�?CRC8 �?*/ // 每行注释
    uint8_t receivedCRC = 0;                                        /* 接收到的 CRC8 �?*/ // 每行注释
    uint8_t *buf = dataPacket.data;                                 /* 指向接收数据缓冲区的指针 */ // 每行注释
    uint16_t len = dataPacket.length;                                /* 接收数据的长�?*/ // 每行注释

    /* 检查最小长度：�?1)+功能�?1)+执行索引(2)+CRC(1) = 5 */ // 每行注释
    if (len < 5)                                                    /* 长度不足，直接返�?*/ // 每行注释
    {
        QueueSendfmt(xReceiveLogQueue, 0, "解析失败：数据长度过�?%d)\r\n", len); /* 记录错误日志 */ // 每行注释
        return;                                                      /* 退出函�?*/ // 每行注释
    }
    ...
    {
        QueueSendfmt(xReceiveLogQueue, 0, "解析失败：包头错�?0x%02X)\r\n", buf[0]); /* 日志包头错误 */ // 每行注释
        return;                                                      /* 退出函�?*/ // 每行注释
    }

    /* 计算 CRC8：调用封装的 CalculateCRC8 函数，计算除最后一字节之外�?CRC */ // 每行注释
    ...
    QueueSendfmt(xReceiveLogQueue, 0, "数据包解析成功: Func=0x%02X, ExecIdx=%u, DataLen=%u, CRC=0x%02X\r\n", parsedDataPacket.FunctionCode, parsedDataPacket.ExecIndex, parsedDataPacket.DataLen, parsedDataPacket.CRC8); /* 发送成功日�?*/ // 每行注释
}
