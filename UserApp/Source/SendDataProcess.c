#include "SendDataProcess.h" // 包含本模块的头文件，声明接口和常量
...
{ // 每行注释
    uint8_t crc = 0x00; // 初始化 CRC
    for (size_t i = 0; i < length; ++i) { // 遍历所有字节
        crc ^= data[i]; // 异或字节
        for (uint8_t bit = 0; bit < 8; ++bit) { // 逐位处理
            if (crc & 0x80) { // 检查最高位
                crc = (uint8_t)((crc << 1) ^ 0x07); // 左移并按多项式异或
            } else {
                crc <<= 1; // 仅左移
            }
        }
...
uint8_t BuildReplyPacket(uint8_t functionCode, uint16_t execIndex, const uint8_t *data, uint8_t dataLen, uint8_t *outBuf, uint16_t *outLen)
{
    ...
    if (dataLen > 0 && data == NULL) {
        return 3; // dataLen>0 但 data 为 NULL
    }
    ...
    uint16_t pos = 0;
    outBuf[pos++] = 0xAA;                    /* 包头 */
    outBuf[pos++] = functionCode;            /* 功能码 */
    outBuf[pos++] = (uint8_t)(execIndex >> 8); /* 执行索引高字节 */
    outBuf[pos++] = (uint8_t)(execIndex & 0xFF); /* 执行索引低字节 */
...
/* 重构后的：封装构建并发送应答包到日志队列
 * - 原实现将二进制包放入 xReceiveLogQueue（文本队列），会被 strlen 截断，导致上位机未收到完整包
 * - 本实现直接通过 CDC_Transmit_FS 发送二进制包，并用文本日志报告发送结果
 */
void ReplyPacket(ReplyCode_t reply)
{
    uint8_t outBuf[PACKET_MAX_SIZE]; // 定义输出缓冲区用于构建数据包
    uint16_t outLen = 0; // 存放构建后的包长度
    uint16_t execIndex = (uint16_t)reply; // 将应答码放入执行索引字段（2 字节）
    ...
    if (buildRet != 0) { // 检查构建结果
        if (xReceiveLogQueue != NULL) { // 若日志队列存在则记录错误
            QueueSendfmt(xReceiveLogQueue, 0, "构建应答包失败，错误码=%u\r\n", buildRet); // 记录构建失败日志
        }
        return; // 构建失败，返回
    }

    /* 如需查看实际构建的字节序列，记录十六进制文本日志（非二进制队列） */
    if (xReceiveLogQueue != NULL) { // 若日志队列存在则格式化打印十六进制表示
        char hexbuf[128]; // 缓冲区用于构建十六进制字符串
        int pos = 0; // 当前写入位置
        for (uint16_t i = 0; i < outLen && pos < (int)sizeof(hexbuf) - 4; ++i) { // 遍历包字节并格式化为字符串
            pos += snprintf(&hexbuf[pos], sizeof(hexbuf) - pos, "%02X ", outBuf[i]); // 将每个字节格式化为 XX 空格
        }
        hexbuf[pos] = '\0'; // 以 null 结尾
    ...
    if (xReceiveLogQueue != NULL) { // 记录失败信息
        QueueSendfmt(xReceiveLogQueue, 0, "发送失败，最后错误=%u\r\n", txRet); // 记录失败信息
    }
}
